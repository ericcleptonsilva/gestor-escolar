                        ...
                        {records.length === 0 ? (
                            ...
                            records.map(r => (
                                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                    ...
                                    <td className="px-4 py-3">
                                        <span className={`px-2 py-0.5 rounded text-xs border ${r.status === 'No Prazo' || r.status === 'Em Dias' || r.status === 'Antecipado'
                                    ...
                                    <tr key={t.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <img src={t.photoUrl} alt="" className="w-10 h-10 rounded-xl object-cover border border-slate-200 dark:border-slate-700 shadow-sm bg-slate-100" />
                                                <span className="font-bold text-slate-700 dark:text-slate-200">{t.name}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-slate-500 font-medium">{t.registration || '-'}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-wrap gap-1 max-w-xs">
                                                {(t.classes || []).length > 0
                                                    ? (t.classes || []).slice(0, 3).map((c, i) => (
                                                        <span key={i} className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 text-[9px] font-black rounded-lg border border-indigo-100 dark:border-indigo-900/30">
                                                            {c.grade}
                                                        </span>
                                                    ))
                                                    : <span className="text-slate-400 italic text-xs">Nenhuma turma</span>
                                                }
                                                {(t.classes || []).length > 3 && <span className="text-[9px] font-black text-slate-400">+{t.classes.length - 3}</span>}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-1">
                                                <button onClick={() => handleOpenTeacherModal(t)} className="p-2 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-colors"><Edit size={16} /></button>
                                                <button onClick={() => handleDeleteTeacher(t.id)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl transition-colors"><Trash2 size={16} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
